...
namespace TestApp
{
    class EquationSolver
    {
        ...
        public void Linear(string equation){
            ...
            float nrSum = nrRight - nrLeft;//type float so solution performs float division, not int
            ...
            if (xSum == 0 && nrSum == 0)
            {
                Random rand = new Random();//just fun
        ...
        private void Evaluate(string fullString, int start, int end, bool side, ref int xRSum, ref int xLSum, ref int nrRSum, ref int nrLSum)
        {
            ...
            string term = fullString.Substring(start, end-start); //SubString zweite zahl länge
            int value;
            if (term.Contains("x"))
            {
